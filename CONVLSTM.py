...
STAMP = 'BiConv%d-(%d,%d)lstm_+Glove840B%d_%d_%.2f_%.2f'%(num_filters,filter_sizes[0], filter_sizes[1], num_lstm, num_dense, rate_drop_lstm, rate_drop_dense)
...
x1 = Conv1D(filters=32, kernel_size=3, padding='same', activation='relu')(es1)
x1 = MaxPooling1D(pool_size=2)(x1)
x1 = shared_lstm(x1)

sequence_2_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
es2 = embedding_layer(sequence_2_input)
y1 = Conv1D(filters=32, kernel_size=3, padding='same', activation='relu')(es2)
